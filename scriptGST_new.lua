...
main = function ()
	...
	while true do		
	    if ( getBat() > 16 ) then
			...
			dash_start 		= dash_start or (CanIn:process()==1) --процесс получение данных с входа Can. Переменная становится единицей, как только что-то получили от приборки
			local start 	= getDIN(ING_IN)	
			local temp     	= ( dash_start ) and ( CanIn:getByte(1) - TEMP_OFFSET ) or 0   -- получаем первый байт из фрейма, температура охлаждающей жидкости
			local OilTemp  	= ( dash_start ) and ( CanIn:getByte(2) - TEMP_OFFSET ) or 0 --  CanOilTempIn:getByte(1)  -- получаем первый байт из фрейма, температура масла
			...
			setOut(OIL_FAN_CH, ( ( OilTemp >30) or (OilTemp == ( -TEMP_OFFSET))) and (not START_ENABLE)  )
			...
			CanToDash:setBit(2, 8, HIGH_BEAM )
			CanToDash:setBit(2, 7, Ligth_Enable)
			CanToDash:setBit(2, 6, Turns:getAlarm())
			CanToDash:setBit(2, 5, RIGTH_ENABLE )
			CanToDash:setBit(2, 4, LEFT_ENABLE)
			CanToDash:setBit(2, 3, PREHEAT)
			CanToDash:setBit(2, 1, REAR_MOVE)
			CanToDash:setBit(2, 2, UP_MOVE)
			CanToDash:setBit(1, 8, not ( REAR_MOVE and UP_MOVE ) )
