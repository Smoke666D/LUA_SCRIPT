...
main = function ()
	...
	local LigthKeyDelay     = Delay:new (2000, false)
	...
	while true do		
	    ...
	    if (( getBat() > 16 ) or (getBat()<6) ) then
            ...
            if (not slef_check) and start then
				slef_check = true
			end
			if slef_check and not start then
				self_check = false
				scFSM = 0
			end
       			
		    if ( (self_check) and ( scFSM <=10 ) ) then
				scFSM = scFSM + 1
				if scFMS== 1 then
					setOut(STOP_VALVE, true)
					setOut(DOWN_GEAR_CH, true)
				 	setOut(UP_GEAR, true)
				elseif scFMC == 10 then
				...
				STOP_SIGNAL_ERROR  = getOut( STOP_CH)       and ( getOutStatus(STOP_CH) > 1 )       	or STOP_SIGNAL_ERROR 
               ...
               -- KeyBoard:setLedBlue(5, not STOP_VALVE_ERROR) 
				--KeyBoard:setLedBlue(6, not REAR_VALVE_ERROR) 
				--KeyBoard:setLedBlue(7, not FORWAR_VALVE_ERROR) 
