...
main = function ()
	...
	local OilFanTimer		= Delay:new(4000, false)
	...
	while true do		
	    ...
	    if (( getBat() > 16 ) or (getBat()<7) ) or getDIN(ING_IN) then
			ALL_OFF()
			restart = true;
			PreheatTimer = 0
		else
			...
			local START_ENABLE = KeyBoard:getKey(1)  --and (RPM < 700)
			...
			local oilfan_start = oil_fan_enable and (not START_ENABLE) 
			...
			GearCounter:process(KeyBoard:getKey(4) and gear_enable,KeyBoard:getKey(8) and gear_enable, restart or  parking_on  )
			...
			local HORN = KeyBoard:getKey(7) 
			setOut(HORN_CH, HORN )
			KeyBoard:setLedGreen(7,HORN )
			--конец блока упрвления горонм
					
			--Блок управления дальним и билжним светом и стоп сигналом
			BeamCounter:process( KeyBoard:getKey(2), false, restart)  -- cчетчик process( инкримент, дикремент, сборс)
			...
			setOut( STOP_CH,  Ligth_Enable or stop_signal)  --ближний свет и стоп сигнал
