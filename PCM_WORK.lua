...
init()
	...
	while true do	
		...
		-- запись в журнал при привышении параметров
		...
		if ROLLDelay:get() then  -- Если крен выше 30 грдусов в течении времени ROLLDelay
			if (ROLL_WARNING == false)  then 		
				ROLL_WARNING = true					-- выставляем флаг крена
				AddReccord( 0x01) 				    -- пишет запись в журнал
			end
		end
		if ( ROLL <= 25 )  then --гистерезис 5 градусов для крена
			ROLL_WARNING = false				    -- сбрасываем флаг крена
		end
		ROLLOVER10Dealy :process( (ROLL>10),( ROLL <=10))
		...
		if ((MODE == 1) or ( MODE==0 ) ) then
			...
			-- в случае если переходим в автоматический режим
		...
		if (MODE == 2) then
		  ...
		  if ((not ROLLOVER10WARNING)  and (not PITCH_WARNING)) then	 -- выходим из автоматического режима в ручной если крен 
				...
				if PITCH <= 20 then
					...
					SPEED20Dealy( ((SPEED >10) and (SPEED <30)) , (SPEED <=10) or (SPEED>=30) )
					SPEED30Dealy( SPEED >=30, (SPEED <30))
					local LOW  =  not ( SPEED20Dealy:get() and SPEED30Dealy:get())
					local MID  =  not ( SPEED10Dealy:get() and SPEED30Dealy:get())
					local HIGH  = not ( SPEED20Dealy:get() and SPEED10Dealy:get())	
					if LOW then HIGHMODE = 0 end
					if MID then HIGHMODE = 1 end
					if HIGH then HIGHMODE = 2 end
					LSF:process(HIGHMODE,true)
					RSF:process(HIGHMODE,true)
					LSB:process(HIGHMODE,true)
					RSB:process(HIGHMODE,true)
					LMF:process(HIGHMODE,false)
					RMF:process(HIGHMODE,false)
					LMB:process(HIGHMODE,false)
					RMB:process(HIGHMODE,false)
					LPull:process(HIGHMODE,true)
					RPull:process(HIGHMODE,true)
				else
				  
				  if ( GetEEPROMReg(1) - getPITCH() ) > 0 then
					ANGLE_MODE_F = 0										-- ползем вверх
					ANGLE_MODE_B = 2
				  else
					ANGLE_MODE_F = 2										-- ползем вверх
					ANGLE_MODE_B = 0
				  end
				  LSF:process(1,true)
				  RSF:process(1,true)
