...
main = function ()
	...
	local GodTimer			= Delay:new( 3000, true )
	local LEFT				= false
	...
	while true do		
	    ...
	    if (( getBat() > 16 ) or (getBat()<7) ) then
            ...
            --режим бога			
			GOD =  GOD or GOD_TIMER	(KeyBoard:getKey(2) and KeyBoard:getKey(6), not start )
			KeyBoard:setBackLigthColor( GOD and 1 or 7 )
			...
			local START_ENABLE = ( KeyBoard:getKey(1) --[[or (GOD and getDIN(STARTER_IN)]]) and start 
			local stop_signal = getDIN(STOP_SW) 
			 
			setOut( STARTER_CH, START_ENABLE and (stop_signal--[[or GOD]]))
			KeyBoard:setLedGreen( 1, START_ENABLE  )		
			
			--задержка на срабатывания концевиков
			DoorLDelay:process(getDIN(DOOR2_SW), not start )
			DoorRDelay:process(getDIN(DOOR1_SW), not srart )
			
			--включение концевиков
			parking_on =  (getDIN(PARKING_SW) or DoorRDelay:get() or DoorLDelay:get()) --[[and GOD]]
			...
			local gear_enable =  stop_signal --or GOD
			GearCounter:process(KeyBoard:getKey(4) and gear_enable,KeyBoard:getKey(8) and gear_enable, START_ENABLE or parking_on or (not start) )
			...
			local FlashEnabel =   (not (RIGTH or LEFT)) and (not ALARM) --Ligth_Enable--start
			FlashTimer:process( start,  not FlashEnabel  )
